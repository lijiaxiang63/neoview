...
`src/main/windowLifecycle.ts` holds the pure post-await delivery and close-confirmation gates: sends recheck both window owners and contain the final race, renderer loss bypasses a missing responder, and confirm/cancel state is sender-scoped and released with the window.
