...
import { CorrectionExportController } from './correctionExportController'
...
export default function RuntimeRoot(): JSX.Element {
  ...
  const services = useMemo(() => {
    ...
    const correctionExports = new CorrectionExportController({
      store: useStore,
      bridge: window.neoview,
      storage: localStorage
    })
    ...
    return { runtime, regionExports, correctionExports, updates }
  }, [])
  const { runtime, regionExports, correctionExports, updates } = services
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      updates.dispose()
      correctionExports.dispose()
      regionExports.dispose()
      runtime.dispose()
    }
  }, [correctionExports, regionExports, runtime, updates])
